/* ================================================================
   FolderIcon — 과목 폴더 아이콘 (TS + Tailwind).
   파일명은 types.ts의 Folder 타입과 충돌을 피하려고
   FolderIcon 으로 둔다(기본 export).
   ================================================================ */
...
const Folder: React.FC<FolderProps> = ({ color = '#EAB308', size = 1, items = [], className = '' }) => {
  const maxItems = 3;
  ...
  return (
    <div style={scaleStyle} className={className}>
      ...
      >
        <div
          className="relative h-[80px] w-[112px] rounded-bl-[10px] rounded-br-[10px] rounded-tl-0 rounded-tr-[10px]"
          style={{ backgroundColor: folderBackColor }}
        >
          <span
            className="absolute bottom-[98%] left-0 z-0 h-[10px] w-[36px] rounded-bl-0 rounded-br-0 rounded-tl-[5px] rounded-tr-[5px]"
            style={{ backgroundColor: folderBackColor }}
          ></span>
          {papers.map((item, i) => {
            let sizeClasses = '';
